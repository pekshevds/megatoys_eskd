...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.ЗаголовокФормы= НСтр("ru = 'Заявка на перемещение'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаявкаНаПеремещение") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ЗаявкаНаПеремещение", "Заявка на перемещение", 
			ПечатьДокумента(МассивОбъектов, ОбъектыПечати, ПараметрыПечати),,"Документ.mega_ЗаявкаНаПеремещение.ПФ_MXL_ЗаявкаНаПеремещение");
	КонецЕсли;
...
Функция ПечатьДокумента(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ТабличныйДокумент.КлючПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_ЗаявкаНаПеремещение_ЗаявкаНаПеремещение";
	...
	|	Документ.ПодразделениеЗаказчик,
	|	Документ.ПодразделениеИсполнитель,
	|	Документ.Состав.(
	|		Ссылка КАК Ссылка,
	|		НомерСтроки КАК НомерСтроки,
	|		Номенклатура КАК Номенклатура,
	|		ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		ВидСтадииОбработки КАК ВидСтадииОбработки,
	|		Количество КАК Количество
	|	) КАК Состав
	|ИЗ
	|	Документ.mega_ЗаявкаНаПеремещение КАК Документ
	|ГДЕ
	...
	Пока Шапка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.mega_ЗаявкаНаПеремещение.ПФ_MXL_ЗаявкаНаПеремещение");
		...
		ОбластьМакета.Параметры.Заголовок = 
			СтрШаблон("Заявка на перемещение №%1 от %2", Шапка.Номер, Формат(Шапка.Дата, "ДФ='dd.MM.yyyy ""г""';"));		

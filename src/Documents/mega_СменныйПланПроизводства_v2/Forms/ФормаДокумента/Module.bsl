...
Процедура ПеренестиМатериалыВНовыйДокументНаКлиенте()
	...
	Форма = ПолучитьФорму("Документ.mega_СменныйПланПроизводства_v2.ФормаОбъекта",,, Истина);
...
&НаСервере
Процедура ОбработкаСтрокиРекурсивно(ТекСтрокаСостав)
	...
	Пока ВыборкаМатериалы.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ВыборкаМатериалы.СтадияОбработки) И 
			ЗначениеЗаполнено(ВыборкаМатериалы.Спецификация) Тогда
				
			ОбработкаСтрокиРекурсивно(ВыборкаМатериалы);	
		Иначе
...
Процедура ИнициализацияТаблицМатериаловИПолуфабрикатовИВидовСтадийОбработки()
	...
	Для Каждого ТекСтрокаСостав Из Объект.Состав Цикл
		...
		ОбработкаСтрокиРекурсивно(ТекСтрокаСостав);
	КонецЦикла;

...
Процедура ЗаполнитьПоОстаткамПредыдущихВидовСтадийПроизводства(Команда)
	...
	Если Объект.ТехнологическиеОперации.Количество() > 0 Тогда
		...
		Режим = РежимДиалогаВопрос.ДаНет;
		ОписаниеОповещенияОЗавершении = 
			Новый ОписаниеОповещения("ПослеЗакрытияВопросаОНеобходимостиОчисткиТабличнойЧасти", ЭтотОбъект);	
		ПоказатьВопрос(ОписаниеОповещенияОЗавершении,
...
&НаКлиенте
Процедура ПослеЗакрытияВопросаОНеобходимостиОчисткиТабличнойЧасти(Результат, Параметры) Экспорт

...
&НаСервере
Процедура ЗаполнениеТаблицыСостав(ТаблицаДляПолученияОстатков)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Доступно <= 0 Тогда
		...
		НоваяСтрока.ВидСтадииОбработки = ВыборкаДетальныеЗаписи.ВидСтадииОбработки;
		НоваяСтрока.Спецификация = ВыборкаДетальныеЗаписи.Спецификация;
		НоваяСтрока.НаСкладе = ВыборкаДетальныеЗаписи.НаСкладе;
		НоваяСтрока.ВПроизводстве = ВыборкаДетальныеЗаписи.ВПроизводстве;
		НоваяСтрока.Доступно = ВыборкаДетальныеЗаписи.Доступно;
	КонецЦикла;
КонецПроцедуры
...
Процедура ИнициализацияТаблицыСостав()
	...
	ЗаполнениеТаблицыСостав(ТаблицаДляПолученияОстатков);			
КонецПроцедуры

...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ЗаявкаВнутренняя";
	КомандаПечати.Представление = НСтр("ru = 'Заявка внутренняя'");
	КомандаПечати.ЗаголовокФормы= НСтр("ru = 'Заявка внутренняя'");
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ЗаявкаВнутренняя") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ЗаявкаВнутренняя", "Заявка внутренняя", 
			ПечатьДокумента(МассивОбъектов, ОбъектыПечати, ПараметрыПечати),,"Документ.mega_ЗаявкаВнутренняя.ПФ_MXL_ЗаявкаВнутренняя");
	КонецЕсли;
...
Функция ПечатьДокумента(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	ТабличныйДокумент.КлючПараметровПечати	= "ПАРАМЕТРЫ_ПЕЧАТИ_mega_ЗаявкаВнутренняя_ЗаявкаВнутренняя";
	...
	|	) КАК Состав
	|ИЗ
	|	Документ.mega_ЗаявкаВнутренняя КАК Документ
	|ГДЕ
	...
	Пока Шапка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.mega_ЗаявкаВнутренняя.ПФ_MXL_ЗаявкаВнутренняя");
		...
		ОбластьМакета.Параметры.Заголовок = 
			СтрШаблон("Внутренняя заявка №%1 от %2", Шапка.Номер, Формат(Шапка.Дата, "ДФ='dd.MM.yyyy ""г""';"));		

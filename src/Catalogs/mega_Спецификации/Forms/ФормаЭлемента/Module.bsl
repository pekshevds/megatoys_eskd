...
Процедура ПриОткрытии(Отказ)
	...
	Если ТекущиеДанные <> Неопределено Тогда 		
		...
		УстановитьОтборСтрокСопутствующийВыпуск();
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыСопутствующийВыпуск

&НаКлиенте
Процедура СопутствующийВыпускПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	
	Если НоваяСтрока Тогда
		
		ТекущиеДанные = Элементы.СопутствующийВыпуск.ТекущиеДанные;		
		Если ТекущиеДанные <> Неопределено Тогда
...
&НаКлиенте
Процедура СопутствующийВыпускПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, ЭтоГруппа, Параметр)
...
&НаКлиенте
Процедура СопутствующийВыпускНоменклатураПриИзменении(Элемент)
...
&НаКлиенте
Процедура СопутствующийВыпускНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
...
&НаКлиенте
Процедура УстановитьОтборСтрокСопутствующийВыпуск()
	
	Элементы.СопутствующийВыпуск.ОтборСтрок = Новый ФиксированнаяСтруктура("КлючСвязи", ТекущаяСтадияОбработки);
КонецПроцедуры
...
Функция МожноУдалитьСтрокуСтадииОбработки()
	...
	Возврат Объект.Материалы.НайтиСтроки(Новый Структура("КлючСвязи", ТекущаяСтадияОбработки)).Количество() = 0 И 
			Объект.ТехнологическиеОперации.НайтиСтроки(Новый Структура("КлючСвязи", ТекущаяСтадияОбработки)).Количество() = 0 И
			Объект.СопутствующийВыпуск.НайтиСтроки(Новый Структура("КлючСвязи", ТекущаяСтадияОбработки)).Количество() = 0;
КонецФункции
